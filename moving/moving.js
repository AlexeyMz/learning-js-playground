...
window.onload = function () {
    ...
    var seedElement = document.getElementById('seed_value');
    seedElement.value = currentSeed;

    var onSetSeed = function () {
        var seed = seedElement.value;
        window.location.assign(seed.length == 0 ? '' : '?seed=' + encodeURIComponent(seed));
    };
    document.getElementById('set_seed').onclick = function (e) {
        return onSetSeed();
    };
    document.getElementById('clear_seed').onclick = function (e) {
        window.location.assign('');
    };
    seedElement.onkeyup = function (e) {
        if (e.keyCode == 13) {
            onSetSeed();
            return false;
        } else {
